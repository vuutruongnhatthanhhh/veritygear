...
const ICONS: Record<SocialPlatform, { label: string; path: React.ReactNode }> = {
  facebook: {
    label: "Facebook",
    path: <path d="M13.5 21v-7.5h2.52l.38-3H13.5l.004-1.5c0-.78.075-1.2 1.2-1.2h1.68V4.8h-2.52c-2.4 0-3.24 1.212-3.24 3.249V10.5H9v3h1.62V21h2.88Z" />,
  },
  instagram: {
    label: "Instagram",
    path: (
    ...
    <div className={`flex items-center gap-3 ${className}`}>
      {links.map((link) => (
        <a
          key={link.platform}
          href={link.url}
          aria-label={ICONS[link.platform].label}
          target="_blank"
